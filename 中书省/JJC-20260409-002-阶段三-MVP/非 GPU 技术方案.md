...
| 模型 | 参数量 | 内存需求 | CPU 速度 | EX 准确率 | 推荐度 |
|------|--------|---------|---------|----------|--------|
| **SQLCoder-1.5B** | 1.5B | ~3GB | ~5 tokens/s | ~70% | ⭐⭐⭐⭐ |
| **SQLCoder-3B** | 3B | ~6GB | ~3 tokens/s | ~75% | ⭐⭐⭐⭐⭐ |
| TinyLlama-1.1B | 1.1B | ~2GB | ~8 tokens/s | ~60% | ⭐⭐⭐ |
| Qwen2.5-1.5B | 1.5B | ~3GB | ~6 tokens/s | ~65% | ⭐⭐⭐ |
...
class CachedInference:
    def __init__(self):
        self.cache = {}  # question_hash -> sql
    
    @lru_cache(maxsize=1000)
    def generate_with_cache(self, question_hash, question, schema):
...
| 阶段 | 交付物 | 验收标准 |
|------|--------|---------|
| **第 1 周** | Prompt 模板文档、Few-shot 示例集、测试报告 | EX≥70% |
| **第 2 周** | ONNX 模型、推理代码、性能报告 | 响应<10 秒 |
| **第 3 周** | 集成系统、评估报告、Bug 清单 | 100 条测试通过 |
...
| 角色 | 人数 | 职责 | 工时 |
|------|------|------|------|
| **算法工程师** | 1 | Prompt 设计、模型选型、ONNX 导出 | 6 人天 |
| **后端工程师** | 1 | 推理集成、性能优化、系统部署 | 5 人天 |
| **测试工程师** | 1 | 测试用例、准确率评估、Bug 跟踪 | 3 人天 |
...
| 风险项 | 概率 | 影响 | 应对措施 |
|--------|------|------|---------|
| **Prompt 准确率低** | 中 | 高 | 增加 Few-shot 示例（20→30 个）、优化模板结构 |
| **CPU 推理响应慢** | 中 | 中 | INT8 量化、批处理优化、缓存热点查询 |
| **ONNX 导出失败** | 低 | 高 | 备用方案：直接使用 PyTorch + TorchScript |
...
| 风险项 | 概率 | 影响 | 应对措施 |
|--------|------|------|---------|
| **Few-shot 示例不足** | 中 | 中 | 从 Spider/BIRD 数据集抽取、合成生成 |
| **Schema 设计不合理** | 低 | 高 | 参考尚书省阶段二财报样本、专家评审 |
| **测试数据覆盖不全** | 中 | 中 | 增加测试用例（100→150 条）、覆盖边界情况 |
...
| **性能优化不达预期** | 中 | 高 | 降级目标（<15 秒）、启动 GPU 备选方案 |
| **人员变动** | 低 | 高 | 文档化进度、知识共享 |
...
**对比原方案（38 人天）：** 节约 ¥19,000

### 5.3 总成本对比

| 成本项 | 原方案 (GPU) | MVP 方案 (CPU) | 节约 |
|--------|-------------|---------------|------|
| **人力成本** | ¥38,000 | ¥19,000 | ¥19,000 |
| **硬件成本 (月)** | ¥2,000 | ¥500 | ¥1,500 |
| **硬件成本 (年)** | ¥24,000 | ¥6,000 | ¥18,000 |
| **总成本 (首年)** | **¥62,000** | **¥25,000** | **¥37,000** |
...
| 指标 | 目标 | 测量方式 |
|------|------|----------|
| **P50 响应时间** | <5 秒 | 100 次查询 |
| **P95 响应时间** | <10 秒 | 100 次查询 |
| **缓存命中率** | >50% | 重复查询测试 |
| **并发支持** | 1-2 | 压力测试 |
...
| 文档 | 字数 | 说明 |
|------|------|------|
| `docs/prompt-templates.md` | ~3,000 | Prompt 模板设计 |
| `docs/cpu-inference-guide.md` | ~2,000 | CPU 推理集成指南 |
| `docs/mvp-test-report.md` | ~5,000 | MVP 测试报告 |
...
| 数据 | 数量 | 说明 |
|------|------|------|
| Few-shot 示例 | 15 个 | 财务领域 SQL 示例 |
| 测试数据集 | 120 条 | 简单 + 复杂查询 |
| ONNX 模型 | 1 个 | SQLCoder-3B INT8 量化版 |
...
中书省已完成阶段三非 GPU 技术方案（MVP）起草，现将材料提交审议，恭请圣裁。

**审议材料：**
1. Prompt Engineering 方案（模板 + 示例 + 测试计划）
2. CPU 推理方案（模型选型 + ONNX 集成 + 性能优化）
3. 实施计划（4 周/15 人天/人员配置）
4. 风险评估报告（技术/数据/工期风险）

**申请事项：**
- 请门下省审议非 GPU 技术方案
- 请确认 Prompt Engineering + CPU 推理路线是否可行
- 请批复是否按本方案执行

**中书省 谨上**  
天启二年四月十二日（2026 年 4 月 12 日）22:45

---

**中书省 谨记**  
天启二年四月十二日（2026 年 4 月 12 日）22:45  
**阶段三非 GPU 技术方案（MVP）起草完成，提交门下省审议！** 📋
